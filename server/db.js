...
export const initDb = async () => {
  ...
  // Migration: Add share_uuid to estimates if it doesn't exist
  try {
    db.run("ALTER TABLE estimates ADD COLUMN share_uuid TEXT");
    saveDb();
    console.log('✓ Successfully added share_uuid column to estimates table');
  } catch (err) {
    // Column likely already exists
    // console.log('Migration note (share_uuid):', err.message);
  }
  ...
  // Migration: Add priority columns to requests if not exists
  try {
    const reqTableInfo = db.exec("PRAGMA table_info(requests)");
    const reqColumns = reqTableInfo[0]?.values.map(col => col[1]) || [];
    if (!reqColumns.includes('priority')) {
      db.run("ALTER TABLE requests ADD COLUMN priority TEXT CHECK(priority IN ('High', 'Medium', 'Low'))");
      saveDb();
      console.log('✓ Added priority to requests');
    }
    if (!reqColumns.includes('presale_priority')) {
      db.run("ALTER TABLE requests ADD COLUMN presale_priority TEXT CHECK(presale_priority IN ('High', 'Medium', 'Low'))");
      saveDb();
      console.log('✓ Added presale_priority to requests');
    }
  } catch (err) {
    console.log('Migration note (priority columns):', err.message);
  }
...
export const insertRequest = (requestData) => {
  ...
  return queryOne(`
    SELECT requests.*, leads.client_name, users.name as creator_name
    FROM requests
